...
### Per-team blackout dates

Currently blackout dates are per-league only. Add support for per-team blackout dates in `team_overrides`, e.g.:
...
This would let teams in a single-team league (like SMN) use blackout dates instead of (or in addition to) the `weekday_only` + `available_weekends` workaround. Implementation touches `config.py` (parse into `Team`), `scheduler.py` (`build_calendar` and `_get_field_candidates` would check team-level blackouts alongside league blackouts), and `constraints.py`.
